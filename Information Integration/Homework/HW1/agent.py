...
def scrape_painting(name, _id, url, soup):
    object = {}
    object['name'] = name
    object['_id']  = _id
    object['link'] = url
    ...
    #on view
    on_view = soup.find('span', {'class': 'on-view-status'}).next_sibling
    object['on_view'] = on_view.next_sibling.strip() if on_view else 'Not currently on public view'
...
def scrape_page(ROOT_URL, hrefs):
    coll = []
    for (name, href, _id) in hrefs:
        URL = ROOT_URL + href
#        URL = r"http://collections.lacma.org/node/201945"
        try:
            response = requests.get(URL)
            soup = BeautifulSoup(response.text).find('div',
                    {'id': 'content'})
            result = scrape_painting(name, _id, URL, soup)
            coll.append(result)
        except:
            print(URL)
            print('-'*60)
...
def paginate(total, ROOT_URL, URL_1, URL_2):
    result = []
    for i in range(total):
        ...
        result.extend(scrape_page(ROOT_URL, hrefs))
#        break
...
def main():
    ...
    SEARCH_URLS = [
        r"&f[0]=bm_field_has_image%3Atrue&f[1]=im_field_curatorial_area%3A32&f[2]=im_field_classification%3A22",
        r"&f[0]=bm_field_has_image%3Atrue&f[1]=im_field_classification%3A22&f[2]=im_field_curatorial_area%3A41"]
    for SEARCH_URL in SEARCH_URLS:
        ...
        total = int(soup.find('div', {'class': 'search-total'}).text[:3]) \
            // 20 + 1
        results.extend(paginate(total, ROOT_URL, QUERY_URL, SEARCH_URL))
#        break
    try:
